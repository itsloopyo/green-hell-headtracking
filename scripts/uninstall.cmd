@echo off
:: ============================================
:: Green Hell Head Tracking - Uninstall
:: ============================================
...
set "LEGACY_DLLS=GreenHellHeadTracking.pdb CameraUnlock.Core.dll CameraUnlock.Core.Unity.dll CameraUnlock.Core.Unity.Harmony.dll"
set "GOG_IDS="
set "SEARCH_DIRS="
:: --- END CONFIG BLOCK ---
...
set "PLUGINS_PATH=%GAME_PATH%\Mods"
set "REMOVED=0"
...
:: --- Check if we should remove MelonLoader ---
if "!FORCE!"=="1" (
    echo.
    echo Removing MelonLoader ^(--force^)...
...
if "!WE_INSTALLED!"=="0" (
    echo.
    echo MelonLoader was not installed by this mod - leaving intact. Use --force to remove anyway.
    goto :cleanup_state
)

echo.
echo Removing MelonLoader ^(it was installed by this mod^)...
...
for %%f in (version.dll dobby.dll) do (
